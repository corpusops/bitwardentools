...
PRIVATE_KEY = os.environ.get("BITWARDEN_PRIVATE_KEY")
...
def get_reverse_cache():
    ...
    for k in "by_collection", "by_organization":
        for container in CACHE["ciphers"][k]:
            rciphers.extend(
                get_all_cached_items(CACHE["ciphers"][k][container]))
    return {
...
class Client(object):
    ...
    def unlock(self, force=False):
        if (self.session is None) or force:
            ret = self.call(
                f"unlock {self.password}", no_session=True, asjson=False
            ).stdout.decode()
            s = (
                [a for a in ret.splitlines() if "BW_SESSION=" in a][0]
                .split()[-1]
                .split('"')[1]
            )
            self.session = s
    ...
    def call(
        ...
        if sync:
            self.cli_sync()
        if not no_session:
            os.environ["BW_SESSION"] = self.unlock(force=force_unlock)
        if input is None:
        ...
        if ret.returncode != 0:
            raise RunError(ret.stdout + ret.stderr)
        if asjson:
            try:
    ...
    def sync(self, sync=None, token=None):
        return self.api_sync(sync=sync, token=token), self.cli_sync(sync=sync)
    ...
    def get_organization(self, orga, sync=None, cache=None, token=None, complete=None):
        ...
        try:
            ...
            pass
        try:
            orgas = self.finish_orga(
                organizations["name"][_id], token=token, cache=cache, complete=complete
            )
            if len(orgas) > 0:
    ...
    def get_organization_key(self, orga, token=None, sync=True):
        ...
        if not nvalue:
            ...
            for i, v in value.items():
                try:
                    ...
                    v, orga=orga, key=key, token=token, recursion=recursion, dictkey=i
                    )
                except:
                    raise
    ...
    def link(self, ciphers, relcollections, collections=None, orga=None, token=None, link=True):
        ret = []
        token = self.get_token(token)
    ...
    def search_objects(self, json_or_obj, types=None, sync=False, limit=None, **kw):
        ...
        '''
        ret = []
        for ix, ((typ, i), obj) in enumerate(self.search(
            json_or_obj, types=types, sync=sync, **kw
        ).items()):
