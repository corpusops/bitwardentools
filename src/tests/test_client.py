...
import unittest
import traceback
from bitwardentools import common, client as bwclient, crypto as bwcrypto
...
class TestBitwardenInteg(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = bwclient.Client(email=email, password=pw, login=False)
        self.setup_users()
        self._wipe()
    ...
    def setup_users(self):
        for attr, email, password in [
            ...
            # last login will keep the client loggued as admin
            ('admin', self.email, self.password),
        ]:
            try:
                setattr(self, attr, self.client.get_user(email))
                self.client.login(email, password)
            except (bwclient.LoginError, bwclient.UserNotFoundError):
                try:
    ...
    def setup_fixtures(self):
        ...
        self.osecn = client.create_securenote("sec2", orga, collections=[col], **self.secn)
        self.seccard = {
    ...
    def test_create_via_payload(self):
        client = self.client
        orgp = {'object': 'organization',
                'name': "testorgp",
                'email': "foo@org"}
        orga = client.create(**orgp)
        colp = {'object': 'org-collection',
                'name': "testcolp",
                'organizationId': client.item_or_id(orga)}
        colp2 = {'object': 'org-collection',
                 'name': "testcolp2",
                 'organizationId': client.item_or_id(orga)}
        cipherp = {"object": "item",
                   "name": "testitp",
        ...
        for i in (
            ...
            lambda: (False not in [
                colp[a] == getattr(col, a) for a in ['name', 'organizationId']]),
            lambda: (False not in [
                cipherp[a] == getattr(cipher, a) for a in ['name', 'notes', 'organizationId']]),
            lambda: (False not in [
                orgp[a] == getattr(
                    orga, {'email': 'billingEmail'}.get(a, a))
                for a in ['name', 'email']]),
        ):
            self.assertTrue(i())
    ...
    def test_ciphers(self):
        ...
        # login
        for i in (
            lambda: self.secp['login']['uris'][0]['uri'] == self.osec5.login['uris'][0]['uri'],
            lambda: self.secp['notes'] == self.osec5.notes,
            lambda: (False not in [(self.osec5.login[i] == self.secp['login'][i])
        ...
        for i in (
            lambda: (False not in [
                self.secn[a] == getattr(self.osecn, a) for a in ['notes']]),
            lambda: (False not in [(self.osecn.secureNote[i] == self.secn['secureNote'][i])
                                   for i in ['type']]),
        ):
            self.assertTrue(i())
        # seccard
        for i in (
            lambda: ({'name': 'aaa', 'type': 0, 'value': 'aaa'} ==
                     self.oseccard.card['fields'][0]),
            lambda: (False not in [
                self.seccard[a] == getattr(self.oseccard, a) for a in ['notes']]),
            lambda: (False not in [(
        ...
        for i in (
            lambda: (False not in [
                self.seci[a] == getattr(self.oseci, a) for a in ['notes']]),
            lambda: (False not in [(
    ...
    def test_create_user(self):
        ...
        self.assertTrue(
            rsak.public_key().exportKey('PEM').decode(),
            '-----BEGIN PUBLIC KEY-----')
    ...
    def test_delete_user(self):
        ...
        self.assertRaises(bwclient.UserNotFoundError,
                          self.client.get_user, uid, sync=True)
